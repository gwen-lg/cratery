...
pub enum LaunchError {
    #[error("Failed to create new db with default content")]
    CreateNewDb(#[source] io::Error),
    ...
    #[error("Failed to get Index with")]
    GetIndex(#[source] UnApiError),

    ///TODO: convert to not use `ApiError` in parent
    #[error("Failed to get JobSpecs")]
    JobSpecs(#[source] UnApiError),

    ///TODO: convert to not use `ApiError` in parent
    #[error("TODO: doc generator")]
    ...
    pub async fn launch<P: ServiceProvider>(configuration: Configuration) -> Result<Arc<Self>, LaunchError> {
        ...
        if tokio::fs::metadata(&db_filename).await.is_err() {
            ...
            tokio::fs::write(&db_filename, DB_EMPTY)
                .await
                .map_err(LaunchError::CreateNewDb)?;
        }
...
impl AuthenticationError {
    ...
    fn code(&self) -> u16 {
        match self {
            AuthenticationError::Unauthorized | AuthenticationError::CookieMissing => 401,
            AuthenticationError::CookieDeserialization(_error) => 500,
            AuthenticationError::GlobalToken(_error) => 500,
            AuthenticationError::UserToken(_error) => 500,
            AuthenticationError::CheckUser(_error) => 500,
            AuthenticationError::NoUserAuthenticated => 400,
        }
    }
