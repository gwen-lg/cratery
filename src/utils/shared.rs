...
pub(crate) struct StillSharedError(usize);
...
#[derive(Debug)]
pub(crate) struct ResourceLock<'r, R> {
...
pub(crate) struct SharedResource<R> {
    /// The inner resource
    inner: Arc<Mutex<R>>,
}
...
impl<R> SharedResource<R> {
    ...
    pub(crate) async fn borrow(&self) -> ResourceLock<'_, R> {
    ...
    pub(crate) fn into_original(self) -> Result<R, StillSharedError> {
