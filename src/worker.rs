...
pub enum WorkerError {
    ...
    ApiError { message: String, detail: Option<String> },
}
...
pub async fn main_worker(config: Configuration) -> Result<(), WorkerError> {
    ...
    let ws = main_worker_connect(worker_config, &descriptor)
        .await
        .map_err(|source| WorkerError::ApiError {
            message: source.message,
            detail: source.details,
        })?;
    main_loop(ws, &descriptor, config)
        .await
        .map_err(|source| WorkerError::ApiError {
            message: source.message,
            detail: source.details,
